...
- Status: working (curated pin only) — `lego-roses` ingested for real 2026-07-24 (`node tools/crawl.mjs --shop Ringo` → POST /api/ingest 200 `{"ok":true,"ingested":1}`, kr 830 live on pricy.no). Full-catalog sitemap discovery also tried (`$discover` wired against `https://www.ringo.no/sitemap_index.xml`, 5 product sub-sitemaps, ~5000 URLs total): 0/30 sampled product pages carry a `gtin` in their JSON-LD — this shop just doesn't publish EAN/GTIN data at all. Left wired in case that changes; not worth a full crawl at 0% yield today.
