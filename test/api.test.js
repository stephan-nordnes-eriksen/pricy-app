...
test('login issues an HttpOnly session cookie and /api/me returns the user', async () => {
  const call = api({ DB: d1() });
  assert.strictEqual((await call('/api/me')).status, 401, 'unauthenticated /api/me must 401');

  const login = await call('/api/auth/login', { method: 'POST', body: { email: 'Ola@Nordmann.no' } });
  ...
  const me = await (await call('/api/me', { cookie: cookieOf(login) })).json();
...
test('bad email is rejected on both auth endpoints', async () => {
  const call = api({ DB: d1() });
  for (const p of ['/api/auth/login', '/api/auth/request']) {
...
test('watchlist persists per user and requires auth', async () => {
  ...
  const ola = cookieOf(await call('/api/auth/login', { method: 'POST', body: { email: 'ola@nordmann.no' } }));
  ...
  const kari = cookieOf(await call('/api/auth/login', { method: 'POST', body: { email: 'kari@example.no' } }));
...
test('logout kills the session and clears the cookie', async () => {
  const call = api({ DB: d1() });
  const cookie = cookieOf(await call('/api/auth/login', { method: 'POST', body: { email: 'ola@nordmann.no' } }));
