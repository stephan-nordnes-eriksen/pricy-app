...
function App(){
  ...
  return (<React.Fragment>
    <div className="app-shell" key={name+JSON.stringify(params)+t.loginLayout+t.homeLayout+t.filterLayout+t.density+t.sparklines+t.plan}>{view}{!({login:1,landing:1,about:1,onboarding:1})[name] && <Footer go={go} />}<CompareTray go={go} hidden={!!({login:1,landing:1,about:1,onboarding:1,compare:1})[name]} /></div>
    <TweaksPanel>
      <TweakSection label="Preview" />
      <TweakSelect label="Screen" value={name} options={[{value:"results",label:"Search results"},{value:"product",label:"Product page"},{value:"shop",label:"Shop profile"},{value:"compare",label:"Compare products"},{value:"home",label:"Signed-in home"},{value:"browse",label:"Categories browse"},{value:"alerts",label:"Alerts / watchlist"},{value:"lists",label:"Lists"},{value:"optimizer",label:"Basket optimizer"},{value:"autobuy",label:"Auto-buy orders"},{value:"account",label:"Account & settings"},{value:"onboarding",label:"Onboarding"},{value:"login",label:"Login"},{value:"landing",label:"Public landing"},{value:"about",label:"About (public)"}].filter(o=>o.value!=="autobuy"||!t.hideAutobuy)} onChange={(v)=>{ if(v==="compare") CompareStore.seed(["xm5","bose-ultra","senn-m4"]); go(v, v==="results"?{cat:"Audio"}:v==="product"?{id:"xm5"}:v==="shop"?{shop:"Elkjøp"}:{}); }} />
